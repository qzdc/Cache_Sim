...
void output()
{
    ...
    fout << "Total Hit Rate: " << fixed << setprecision(4) << 100.0 * (l_hit + s_hit) / (l_num + s_num) << '%' << endl;
    fout << "Load Hit Rate: " << fixed << setprecision(2) << 100.0 * l_hit / l_num << '%' << endl;
    fout << "Store Hit Rate: " << fixed << setprecision(2) << 100.0 * s_hit / s_num << '%' << endl;
    fout << "Total Run Time: " << runtime << endl;
    fout << "AVG MA Latncy: " << fixed << setprecision(2) << 1.0 * runtime / (l_num + s_num);
    fout.close();
}